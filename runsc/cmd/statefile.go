...
// Statefile implements subcommands.Command for the "state" command.
...
func (s *Statefile) SetFlags(f *flag.FlagSet) {
	...
	f.StringVar(&s.output, "output", "", "target to dump the statefile.")
	f.BoolVar(&s.html, "html", false, "outputs the file in HTML format.")
}
